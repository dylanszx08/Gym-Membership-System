				...
				if (systemChoice == 1) {
					...
					do {
						...
						while (cin.fail() || subChoice < 1 || subChoice > 5) {
							cin.clear();
							cin.ignore(100, '\n');
							cout << "Invalid choice! Enter 1-5: ";	a
							cin >> subChoice;
						}
...
// Helper Function: Calculates subscription fees for a single member (Chap04, Chap09)
double calculateMemberFee(const Member& m) {
	double total = 0.0;

	// Personal Training Subscription Fee (Chap04, Chap09)
	...
	// Coach Training Subscription Fee (Compatible with spelling variations)
...
// Robust Input Validation (Chap02, Chap07)
int getValidChoiceStudentD(int min, int max) {
	int input;
	cin >> input;
	while (cin.fail() || input < min || input > max) {
		cin.clear();            // Clear input failure flag
		cin.ignore(1000, '\n'); // Clear stream buffer
...
// Module Sub-menu controller (Chap05, Chap07)
void reportingModule(const Member members[], int memberCount) {
	int choice;
	do {
		system("cls");
...
// 1. Generate Summary Report (Chap05, Chap08)
void generateSummaryReport(const Member members[], int memberCount) {
	system("cls");
	...
	// Traverse array to aggregate membership counts
	for (int i = 0; i < memberCount; i++) {
	...
	// Traverse shared bookings array to aggregate active bookings
	for (int i = 0; i < booking_count; i++) {
...
// 2. Generate Detailed Report (Chap02, Chap08)
void generateDetailedReport(const Member members[], int memberCount) {
	system("cls");
	cout << "============================================================================================\n";
...
// 3. Calculate Statistics (Corrected Pointer/Reference conversion bug) (Chap05, Chap08)
void calculateStatistics(const Member members[], int memberCount) {
	system("cls");
	if (memberCount == 0) {
	...
	// ✅ FIXED: Correctly pass the single Member object at index 0 as baseline
	...
	for (int i = 0; i < memberCount; i++) {
		...
		// Linear search for highest and lowest elements (Chap08)
...
// 4. Sort Records using Bubble Sort (Chap08)
void sortRecords(const Member members[], int memberCount) {
	system("cls");
	if (memberCount == 0) {
	...
	// Create a temporary copy array to avoid changing database order (Chap08)
...
// 5. Display Analysis: Popularity and Heatmap frequency algorithms (Chap05, Chap08)
void displayAnalysis(const Member members[], int memberCount) {
	system("cls");
	int personalCount = 0;
	int coachCount = 0;
	...
	// Determine the most popular service using comparison (Chap04)
	...
	// Weekday Booking Heatmap (using 1D Frequency Array) (Chap08)
